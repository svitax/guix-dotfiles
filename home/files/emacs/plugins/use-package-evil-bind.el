;;; use-package-evil-bind.el --- Extra keywords for use-package.  -*- lexical-binding: t; -*-

;; Extend `use-package-keywords'.
(add-to-list 'use-package-keywords :evil-bind t)

;; Validate and return the args passed to the keyword.
(defun use-package-normalize/:evil-bind (name keyword args)
  "Custom use-package keyword :evil-bind. I use this to provide something
similar to `:bind', but with two additional features that I miss from the
...
to trigger `help-map'. This fails using the default `:bind', meaning that I have
to fall back to calling 'bind-key' manually if I want to assign a prefix.
...
STATE is an evil state. It can be nil or omitted entirely. If given, it should
be an argument suitable for passing to `evil-define-key' -- meaning a symbol
like 'normal', or a list like '(normal insert)'."
 (setq args (car args))
  (unless (listp args)
...
;; Handle the keyword -- expand into the code you want it to run.
(defun use-package-handler/:evil-bind (name _keyword args rest state)
  "Handler for ':evil-bind' use-package extension. See 'use-package-normalize/:evil-bind' for docs."
...
;;; use-package-evil-bind.el ends here
